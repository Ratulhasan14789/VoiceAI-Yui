...
def denlock1(abc,key):
#	if type=='en':
	print(abc)
	res = [list for x in key for list in abc if list[1] == x]
	out=[]
	for i in res:
...
def decrypt(text,key):
	key=reverser(key)
	code1= c_split(len_split(text))
	seq2=[1,2,3,4,5,6,7,8]
	output=''
	for key_c in key:
		seq= denlock2('',key_c,1)
		print(seq)
		for aa in code1: #['2', '8', '1', '7', '4', '3', '5', '6']
			alif=[]
			for c in aa:
				alif.append([c,seq[aa.index(c)]])
				print(alif)
			alif.sort(key = sortSecond)
			print(alif,'\n')
			code1[code1.index(aa)] =[i[0] for i in alif]
			print(code1,'\n')
...
light=encrypt('12345678','344')


print('\n\n========\n\n]')

dark=decrypt(light,'344')
